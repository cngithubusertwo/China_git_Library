...
int SqlMysqlApi::InitGlobalOptions()
{
	...
	int rc = 0;
	const char *max_allowed_packet_str = NULL;

	// Set options in dedicated connection before all other connections
	if(max_allowed_packet_str != NULL)
	{
		rc = Connect(NULL);

		if(rc != -1)
		{
			rc = ExecuteNonQuery(std::string("SET GLOBAL max_allowed_packet=").append(max_allowed_packet_str).c_str(), NULL);
			Disconnect();
		}
	}
...
int SqlMysqlApi::local_infile_read(char *buf, unsigned int buf_len)
{
	...
	for(size_t i = _ldi_current_row; i < _ldi_rows_count; i++, _ldi_current_row++)
	{
		...
		for(size_t k = _ldi_current_col; k < _ldi_cols_count; k++, _ldi_current_col++)
		{
			...
			if((_ldi_cols[k]._native_fetch_dt == SQLT_STR ||
			...
			if(_ldi_cols[k]._native_fetch_dt == SQLT_DAT)
			{
				if(remain_len >= 19)
				{
...
void SqlMysqlApi::FindMysqlPaths()
{
	...
	const char *keys[2] = { "Software\\MySQL AB", "Software\\WOW6432Node\\MySQL AB" };
